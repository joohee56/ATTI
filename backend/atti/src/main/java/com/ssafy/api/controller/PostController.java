...
//		return new ResponseEntity<Post>(test, HttpStatus.OK);
	...
	/*
	 * @PutMapping("/update") // 게시글 수정 public ResponseEntity<String>
	 * editPost(@RequestBody PostUpdateReq editPostInfo){
	 * 
	 * System.out.println("=====================");
	 ...
	 * 
	 * User user = new User(); user.setUserId(editPostInfo.getUserId());
	 * editPost.setUser(user);
	 * 
	 * editPost.setPostTitle(editPostInfo.getPostTitle());
	 * editPost.setPostContent(editPostInfo.getPostContent());
	 * 
	 * postService.editPost(editPost); return new ResponseEntity<String>("SUCCESS",
	 * HttpStatus.OK); }
	 */
